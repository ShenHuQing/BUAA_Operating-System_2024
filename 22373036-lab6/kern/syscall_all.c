...
int sys_write_dev(u_int va, u_int pa, u_int len) {
	...
	if (!(pa >= 0x180003f8 && pa + len - 1 < 0x18000418) && !(pa >= 0x180001f0 && pa + len - 1 < 0x180001f8)) {
		return -E_INVAL;
	}
	//memcpy((void*)(pa | KSEG1), (void*)va, len); //memcpy(void *dst, void *src, len)
	if (len == 1) {
		iowrite8(*(uint8_t*)va, pa | KSEG1);
	}
	else if (len == 2) {
		iowrite16(*(uint16_t*)va, pa | KSEG1);
	}
	else if (len == 4) {
		iowrite32(*(uint32_t*)va, pa | KSEG1);
	}else {
	return -E_INVAL;
	}
	return 0;
...
int sys_read_dev(u_int va, u_int pa, u_int len) {
	...
	if (len == 1) {
		*(uint8_t*) va = ioread8(pa | KSEG1);
	}
	else if (len == 2) {
		*(uint16_t*) va = ioread16(pa | KSEG1);
	}
	else if (len == 4) {
		*(uint32_t*) va = ioread32(pa | KSEG1);
	} else {
	return -E_INVAL;
	} 
	return 0;
